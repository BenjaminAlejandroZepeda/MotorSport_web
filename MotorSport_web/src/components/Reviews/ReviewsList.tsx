...
export default function ReviewsList() {
  ...
  useEffect(() => {
    ...
    const lista = Object.entries(raw).flatMap(([usuarioRaw, vehiculos]) => {
      ...
      return Object.entries(vehiculos as Record<string, number>).map(
        ([vehiculoId, estrellas]) => ({
          usuario,
          vehiculoId,
          estrellas,
        })
  ...
  return (
    ...
    >
      {reseñas.map((r, index) => (
        <div
          key={`${r.usuario}-${r.vehiculoId}-${index}`}
          className="card"
          style={{ padding: "1rem" }}
        >
          ...
          <div style={{ fontSize: "1.5rem", color: "#FFD60A" }}>
            {[1, 2, 3, 4, 5].map((star) => (
              <span
                key={star}
                style={{ color: star <= r.estrellas ? "#FFD60A" : "#ccc" }}
              >
                ★
              </span>
            ))}
